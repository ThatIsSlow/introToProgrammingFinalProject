...
def RunWebd():
    ...
    # this is the bit that actually displays the findings in tkinter, uses several variables to keep track of how to group items together
    ...
    for i in Webdriverv.datafound:    
        ...
        if counter == 3:
            ...
            team_label.grid(column=colum_counter, row=row_counter)
            window.update()
            counter = 0
            colum_counter += 1
            if colum_counter > 5:
...
find_team_but = ttk.Button(mainframe, text="Find My Team!", command=RunWebd)
